...
b = s3.Bucket('chokeadityanovember')
count_image = 0
df = pd.DataFrame()
...
for img_file in b.objects.all():
	...
	if img != "":
		name, ext = img.split('.') 	
		cmd  = "aws rekognition detect-faces --image \'{\"S3Object\":{\"Bucket\":\"chokeadityanovember\",\"Name\":\""+path+"/"+img+"\"}}\' --attributes \"ALL\" --region us-west-2 > /home/ubuntu/choke_json/out_"+name+".json"
		...
		if (len(data['FaceDetails']) == 0 or len(data['FaceDetails'][0]) < 15):
			...
			df = df.append(df_temp)
			df.to_csv('AWSFeatures_ChokePoint_Adi_Nov.csv')
